...
let textAreaContent=document.querySelector('#text-area');
let minutesTag=document.querySelector('#min');
let secondsTag=document.querySelector('#sec');
let milliSecTag=document.querySelector('#m-sec');
let minutes=0;
let seconds=0;
let milliSec=0;
let count=0;
...
let startTimer=()=>
        {
           ...
           milliSec=Math.floor(count-(seconds*100)-(minutes*6000));
           minutesTag.innerText=zero(minutes);
           secondsTag.innerText=zero(seconds);
           milliSecTag.innerText=zero(milliSec);       
        }
let zero=(time)=>
           {
            if(time<10)
...
{
	let enteredTextLength=textAreaContent.value.length;
	...
	let enterdText=textAreaContent.value;
	let equalText=originalText.substr(0,enterdText.length);
	evaluateText(originalText,enterdText,equalText);
});

let evaluateText=(originalText,enterdText,equalText)=>
{
	if(enterdText==='')
	{
		// grey color
	...
	else if(enterdText===originalText)
	{
		// green
		applyColors('green');
...
{
	textAreaContent.style.borderColor=color;
	textAreaContent.style.boxShadow=`0 0 10px ${color}`;
}
